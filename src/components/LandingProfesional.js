...
const LandingProfesional = (props) => {
    ...
    const mostrarPrioritario = () => {
        if (proximoPrioritario) {
           return <div><Row className="bg-danger align-items-center" style={{ height: '50px',  borderRadius: "0.375rem"}}>
    ...
    const mostrarCantTurnosHoy = () => {
        if (!cantTurnosHoy > 0) {
            return <div className="py-2">
                <Row className="align-items-center" style={{ height: '50px', backgroundColor: "lightseagreen", borderRadius: "0.375rem" }}>
                    <Link to={{pathname:`/turnos`}} className="text-black">Hay {cantTurnosHoy} turnos agendados para hoy.</Link>
                </Row>
            </div>
        }
